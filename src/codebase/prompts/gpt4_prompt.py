...
def create_CELEBA_prompts(content):
    ...
    To do so, consider the attributes present in the above captions regarding the samples with Blonde hair (i.e, the class label "Blonde"). Attributes are all the concepts (e.g, demography related concept etc) other than the class label. For example, key attributes to affecting classifier's performance in the 1st sentence are woman, warm smile, plain background, light makeup, and diamond earnings. So come up with the list of hypotheses based based on all these attributes to reason why a model makes systematic mistakes. For the hypotheses, you should be the following python dictionary template, no extra sentence:
    {hyp_dict}
...
def create_UrbanCars_prompts(content):
    prompt_dict = """
    prompt_dict = {
        "H1_<attribute>": [List of prompts],
        "H2_<attribute>": [List of prompts]
    ...
    prompt = f"""
    Context: Cars classification from images using a deep neural network
    Analysis post training: On a validation set, 
    a. Get the difference between the image embeddings of correct and incorrectly classified samples.
    b. Retrieve the top K sentences from the captions of the images that matches closely to the embedding difference in step a.
    c. The sentence list is given below in the descending order of similarity with the embedding difference:
