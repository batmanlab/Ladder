...
def mitigate_error_slices_urbancars(args):
    args.input_shape = get_input_shape(args.dataset)
    criterion = torch.nn.CrossEntropyLoss(reduction="mean")
    tr_urban_df = pd.read_csv(args.clf_results_csv.format(args.seed, "valid", "urban"))
    va_urban_df = pd.read_csv(args.clf_results_csv.format(args.seed, "test", "urban"))
    tr_country_df = pd.read_csv(args.clf_results_csv.format(args.seed, "valid", "country"))
    va_country_df = pd.read_csv(args.clf_results_csv.format(args.seed, "test", "country"))
    tr_df = pd.concat([tr_urban_df, tr_country_df], axis=1)
    tr_df = tr_df.loc[:, ~tr_df.columns.duplicated()]
    va_df = pd.concat([va_urban_df, va_country_df], axis=1)
    ...
    urban_attrs = pickle.load(open(args.slice_names.format(args.seed, "urban"), "rb"))
    urban_attrs = list(urban_attrs.keys())
    print(urban_attrs)
    country_attrs = pickle.load(open(args.slice_names.format(args.seed, "country"), "rb"))
    country_attrs = list(country_attrs.keys())
    print(country_attrs)
    attrs = urban_attrs + country_attrs
    print(attrs)
    col_name_list = attrs
    col_name = []
    ...
    for col in col_name:
        ...
        hyp_name = f"all_slices_y_ensemble_yes_urban_hyp_{col[0]} | country_hyp_{col[1]}"
    ...
    print("################################################################################################")
    country_df = test_df[test_df["out_put_GT"] == 1]
    max_col = country_df[country_attrs].idxmax(axis=1)
    pred_col = max_col.apply(
        lambda x: f"all_slices_y_ensemble_yes_urban_hyp_{x} | country_hyp_{x}_Predictions_bin")
    country_df['all_slices_y_ensemble_y_pred'] = country_df.apply(lambda row: row[pred_col[row.name]], axis=1)

    urban_df = test_df[test_df["out_put_GT"] == 0]
    max_col = urban_df[urban_attrs].idxmax(axis=1)
    pred_col = max_col.apply(
        lambda x: f"all_slices_y_ensemble_yes_urban_hyp_{x} | country_hyp_{x}_Predictions_bin")
    urban_df['all_slices_y_ensemble_y_pred'] = urban_df.apply(lambda row: row[pred_col[row.name]],
                                                              axis=1)
    test_df = pd.concat([country_df, urban_df], axis=0)
    prediction_col = "all_slices_y_ensemble_y_pred"

    print("#################################### Ground truth slice ###########################################")
    calculate_performance_metrics_urbancars_df_ensemble(
        test_df, prediction_col, split="test", bg_ratio=0.95, co_occur_obj_ratio=0.95, log_file=args.out_file)
