...
def generate_metadata_celeba(data_path):
    ...
    for i, (split, attr) in enumerate(zip(splits, attrs)):
        ...
        yi = 1 if ai[9] == "1" else 0  # Blond_Hair
        gi = 1 if ai[20] == "1" else 0  # Male
...
def generate_metadata_mimic_cxr(data_path, test_pct=0.1, val_pct=0.05):
    ...
    df['age_decile'] = pd.cut(df['anchor_age'], bins=list(range(0, 101, 10))).apply(
        lambda x: f'{x.left}-{x.right}').astype(str)
...
def generate_metadata_cxr_multisite(data_path, test_pct=0.1, val_pct=0.05):
    ...
    train_val_idx, test_idx = train_test_split(df_final.index, test_size=test_pct, random_state=42,
                                               stratify=df_final['y'])
    train_idx, val_idx = train_test_split(
...
def generate_metadata_chexpert(data_path, test_pct=0.15, val_pct=0.1):
    ...
    demo_dir = Path(os.path.join(data_path, "demographics", "CHEXPERT_DEMOGRAPHICS.xlsx"))
    assert (chexpert_dir / 'train.csv').is_file()
    assert (chexpert_dir / 'train/patient48822/study1/view1_frontal.jpg').is_file()
    assert (chexpert_dir / 'valid/patient64636/study1/view1_frontal.jpg').is_file()
    assert (demo_dir).is_file()

    df = pd.concat([pd.read_csv(chexpert_dir / 'train.csv'), pd.read_csv(chexpert_dir / 'valid.csv')],
                   ignore_index=True)
    ...
    details = pd.read_excel(demo_dir, engine='openpyxl')[['PATIENT', 'PRIMARY_RACE']]
    ...
    # Male_0 = Male, White
    # Male_1 = Male, Black
    # Male_2 = Male, Other (Latino / Hispanic)
    # Female_0 = Female, White
    # Female_1 = Female, Black
    # Female_2 = FeMale, Other (Latino / Hispanic )
...
def generate_metadata_breeds(data_path, val_pct=0.1):
    from robustness.tools.breeds_helpers import make_living17, make_entity13, make_entity30, make_nonliving26, \
        print_dataset_info, ClassHierarchy
...
if __name__ == "__main__":
    ...
    # For Waterbird y=1 means waterbird and a=1 means water
    print(args.datasets)
    if args.download:
