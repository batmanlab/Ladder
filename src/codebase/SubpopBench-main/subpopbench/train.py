...
if __name__ == "__main__":
    ...
    parser.add_argument("--pos_weights", default=[20.16], help="positive weights for Pneumothorax")
    # others
    ...
    if args.dataset == "NIH_dataset":
            hparams['image_arch'] = 'resnet_sup_in1k'

            hparams['group_balanced'] = True

            args.es_metric = 'overall:AUROC'
    ...
    if 'CRT' in args.algorithm or 'DFR' in args.algorithm:
        ...
        args.pretrained = f"{args.stage1_folder.format(args.seed)}/model.pkl"
    ...
    # imbalance loss for NIH dataaset
    if args.dataset == "NIH_dataset":
        pos_wt = torch.tensor([args.pos_weights[0]]).to(device)
        if args.algorithm == "GroupDRO":
            criterion = torch.nn.BCEWithLogitsLoss(pos_weight=pos_wt, reduction='none')
        else:
            criterion = torch.nn.BCEWithLogitsLoss(pos_weight=pos_wt)
        algorithm.loss = criterion
    algorithm.to(device)
    ...
    for step in range(start_step, n_steps):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            for split in sorted(split_names):
                if args.dataset == 'NIH_dataset':
                    num10 = curr_metrics[split]['per_group']['y=1,a=0']['n_samples']
                    num11 = curr_metrics[split]['per_group']['y=1,a=1']['n_samples']
                    results[f'{split}_AUROC'] = curr_metrics[split]['overall']['AUROC']
                    results[f'{split}_y=1,a=1'] = curr_metrics[split]['per_group']['y=1,a=1']['accuracy']
                    results[f'{split}_y=1,a=0'] = curr_metrics[split]['per_group']['y=1,a=0']['accuracy']
                    results[f'{split}_y=1'] = (curr_metrics[split]['per_group']['y=1,a=0']['accuracy']*num10 + curr_metrics[split]['per_group']['y=1,a=1']['accuracy']*num11)/(num10+num11)
            ...
            for key in split_names:
                ...
                if args.dataset == 'NIH_dataset':
                    tb_logger.log_value(f"{key}_AUROC", results[f"{key}_AUROC"], step)
                    tb_logger.log_value(f"{key}_y=1,a=1_acc", results[f"{key}_y=1,a=1"], step)
                    tb_logger.log_value(f"{key}_y=1,a=0_acc", results[f"{key}_y=1,a=0"], step)
                    tb_logger.log_value(f"{key}_y=1_acc", results[f"{key}_y=1"], step)
