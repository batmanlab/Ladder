...
class GroupDRO(ERM):
    ...
    def _compute_loss(self, i, x, y, a, step):
        if isinstance(self.loss, torch.nn.BCEWithLogitsLoss):
            y_ = torch.nn.functional.one_hot(y, num_classes=2).float()
            losses = self.loss(self.predict(x), y_)
        else:
...
class ReWeight(ERM):
    ...
    def _compute_loss(self, i, x, y, a, step):
        if isinstance(self.loss, torch.nn.BCEWithLogitsLoss):
            losses = self.loss(self.predict(x), torch.nn.functional.one_hot(y, num_classes=2).float())
...
class CVaRDRO(ERM):
    ...
    def _compute_loss(self, i, x, y, a, step):
        if isinstance(self.loss, torch.nn.BCEWithLogitsLoss):
            per_sample_losses = self.loss(self.predict(x),torch.nn.functional.one_hot(y, num_classes=2).float()).view(-1)
        else:
