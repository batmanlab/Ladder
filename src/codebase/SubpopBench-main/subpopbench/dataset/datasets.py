...
class CUB_200_2011_toy_shape(SubpopDataset):
    CHECKPOINT_FREQ = 300
    INPUT_SHAPE = (3, 224, 224,)

    def __init__(self, data_path, split, hparams, train_attr='yes', subsample_type=None, duplicates=None):
        root = os.path.join(data_path, "CUB_200_2011_toy_shape")
        metadata = os.path.join(data_path, "CUB_200_2011_toy_shape", "metadata.csv")
        transform = transforms.Compose([
            transforms.Resize((int(224 * (256 / 224)), int(224 * (256 / 224)),)),
            transforms.CenterCrop(224),
            transforms.ToTensor(),
...
class NIH_dataset(SubpopDataset):
    N_STEPS = 20001
    CHECKPOINT_FREQ = 1000
    INPUT_SHAPE = (3, 224, 224,)
    def __init__(self, data_path, split, hparams, train_attr='yes', subsample_type=None, duplicates=None,ratio=0.8):
        self.metadata = os.path.join(data_path, "modified_dataset.csv")
        csv = pd.read_csv(self.metadata)
        train_val_df = csv[csv['val_train_split'] == 1]
        split_idx = int(len(train_val_df) * ratio)
        if split == "tr":
            df = train_val_df.iloc[:split_idx]
        if split == "va": # 8/2
            df = train_val_df.iloc[split_idx:]
        elif split == "te":
            df = csv[csv['val_train_split']==0]
        self.df = df
        self.idx = list(range(len(df)))
        self.x = df["new_path"].values.astype(str)
        self.y = df["Pneumothorax"].values
        self.a = df["tube_geq_0p5"].values if train_attr == 'yes' else [0] * len(df["tube_geq_0p5"].values)
